...
Выполнено объединение веток.
Добавлены скриншоты работы.
...
Fetch только проверяет и загружает информацию об изменениях с GitHub,
но не изменяет локальные файлы. Pull сначала выполняет Fetch,
а затем сразу применяет изменения в проект.


## Конфликт и его решение

Был создан конфликт путём изменения одной строки в двух ветках.
После merge VS Code показал конфликт,
после чего был выбран нужный вариант и создан новый коммит.
...
В ходе работы были закреплены навыки использования Git и GitHub через интерфейс VS Code.
Изучены коммиты, ветки, merge, fetch и pull.
Также была отработана тема с конфликтами и оформлением проекта.

## Что я понял(а) про .gitignore

* .gitignore помогает не отправлять лишние файлы в GitHub.
* .env нельзя публиковать.
* Логи обычно не нужны в репозитории.
* venv можно пересоздать.
